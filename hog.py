...
# Rohan Sinha, Krish Rastogi AP CSP Period 2
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for rollNumber in range(0, num_rolls):
        ...
        if currentDiceRoll == 1:
            encounteredOne = True  # sow sad
        else:
            sumOfDiceRolls += currentDiceRoll

    if encounteredOne:  # change score if sow sad encountered
        sumOfDiceRolls = 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls == 0:
        currentTurnScore = picky_piggy(opponent_score)
    else:
        currentTurnScore = roll_dice(num_rolls, dice)
...
def hog_pile(player_score, opponent_score):
    ...
    if player_score == opponent_score:  # if the player and opponent scores are the same, double the current player's score
        return player_score
    else:
        return 0  # don't add anything to player's score because the scores are different
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            numOfDice = strategy0(score0, score1)
            pointsToAdd = take_turn(numOfDice, score1, dice, goal)
            score0 += pointsToAdd
            who = next_player(who)  # not simply calling next_player(who) but also reassigning who to its return value
            score0 += hog_pile(score0, score1)  # checking if scores are the same is already done in hog pile
        elif who == 1:
            numOfDice = strategy1(score1, score0)
            pointsToAdd = take_turn(numOfDice, score0, dice, goal)
            score1 += pointsToAdd
            who = next_player(who)
